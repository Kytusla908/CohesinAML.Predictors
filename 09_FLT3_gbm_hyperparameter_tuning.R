...
gbm_tuned <- train(x=x_train_vst_QN, y=y_train, method="gbm",
                   trControl=trControl, tuneGrid = gbmGrid,
                   metric = "Sens", verbose = T)
print("Time took to compute models:")
print(Sys.time()-start_time)

# saveRDS(gbm_tuned, "OutputTables/FLT3_tuned_gbm_vst_QN_smaller.rds")
# saveRDS(gbm_tuned, "OutputTables/FLT3_tuned_gbm_vst_QN_not_so_small.rds")
gbm_tuned_small <- readRDS("OutputTables/FLT3_tuned_gbm_vst_QN_smaller.rds")
gbm_tuned_big <- readRDS("OutputTables/FLT3_tuned_gbm_vst_QN_not_so_small.rds")
...
# saveRDS(top10_gbm, file = "OutputTables/FLT3_top10_gbm_models_smaller.rds")
# saveRDS(top10_gbm, file = "OutputTables/FLT3_top10_gbm_models_not_so_small.rds")
top10_gbm_small <- readRDS("OutputTables/FLT3_top10_gbm_models_smaller.rds")
top10_gbm_big <- readRDS("OutputTables/FLT3_top10_gbm_models_not_so_small.rds")


performance <- get_performance(gbm_tuned_small, x_test_vst_QN, y_test,
...
performance <- get_performance(gbm_tuned_small, Fischer_vst_QN, Fischer_labels,
                               classes = c("FLT3.Mutant", "FLT3.WT"))
# write_confusion_matrix(performance$confusion_matrix,
...
for (i in 1:5) {
  ...
  performance_df <- get_performance(top10_gbm_small[[i]], Fischer_vst_QN, Fischer_labels,
                                    classes = c("FLT3.Mutant", "FLT3.WT"))
...
ggplot(roc_5_df, aes(x = fpr, y = tpr, color = model)) +
  ...
  labs(title = "ROC Curves for Top 5 gbm models for FLT3 mutation on Fischer data",
       x = "False Positive Rate", y = "True Positive Rate",
       color = "Model") +
  theme_minimal() +
  theme(panel.background = element_rect(fill = "white", color = NA),
        plot.background  = element_rect(fill = "white", color = NA))
# ggsave("plots/FLT3/ROC_Fischer_top5_gbm_vst_QN_3000.png", device = "png", width = 18, height = 15,
#        units = "cm", pointsize = 10, dpi = 500)

performance <- get_performance(top10_gbm_small[[5]], Fischer_vst_QN, Fischer_labels,
                               classes = c("FLT3.Mutant", "FLT3.WT"),
