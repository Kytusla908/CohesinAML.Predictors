...
dir.create("plots/FLT3_ITD", recursive = TRUE, showWarnings = FALSE)
...
TCGA_FLT3_mut_type$FLT3_ITD <- ifelse(
  (TCGA_FLT3_mut_type$`Mutation Type` == "In_Frame_Del" | TCGA_FLT3_mut_type$`Mutation Type` == "In_Frame_Ins") &
    TCGA_FLT3_mut_type$coord_start >= 570 &
    (is.na(TCGA_FLT3_mut_type$coord_end) | TCGA_FLT3_mut_type$coord_end <= 620),
  "Mutant", "WT")
...
BEAT_FLT3_mut_type$FLT3_ITD <- ifelse(
  (BEAT_FLT3_mut_type$`Mutation Type` == "In_Frame_Del" | BEAT_FLT3_mut_type$`Mutation Type` == "In_Frame_Ins") &
    BEAT_FLT3_mut_type$coord_start >= 570 &
    (is.na(BEAT_FLT3_mut_type$coord_end) | BEAT_FLT3_mut_type$coord_end <= 620),
  "Mutant", "WT")


# Ensure no repeated samples (some has both ITD and TKD) ==============
TCGA_BEAT_mut_table$Sample_ID <- row.names(TCGA_BEAT_mut_table)
FLT3_all <- rbind(TCGA_FLT3_mut_type[, c("Sample ID", "FLT3_ITD")],
                  BEAT_FLT3_mut_type[, c("Sample ID", "FLT3_ITD")])
FLT3_all <- FLT3_all %>%
  group_by(`Sample ID`) %>%
  summarise(FLT3_ITD = ifelse(any(FLT3_ITD == "Mutant"), "Mutant", "WT"),
            .groups = "drop")
...
TCGA_BEAT_mut_table$FLT3_ITD[is.na(TCGA_BEAT_mut_table$FLT3_ITD)] <- "WT"
TCGA_BEAT_mut_table$Sample_ID <- NULL


# Get labels for FLT3 mutations =================
TCGA_BEAT_FLT3_ITD_labels <- paste0("FLT3_ITD.", TCGA_BEAT_mut_table$FLT3_ITD)
TCGA_BEAT_FLT3_ITD_labels <- as.factor(TCGA_BEAT_FLT3_ITD_labels)
table(TCGA_BEAT_FLT3_ITD_labels)

# Plot mutants proportions
flt3_tab <- as.data.frame(table(TCGA_BEAT_FLT3_ITD_labels))
...
# ggsave("plots/FLT3_ITD/FLT3_ITD_mutants_proportions_TCGA_BEAT.png", device="png",
#        units="cm", dpi=500, width=15, height=12)
# write.table(TCGA_BEAT_FLT3_ITD_labels, "InputTables/TCGA_BEAT_labels_FLT3_ITD.txt",
...
# Splitting Labels
y_train <- as.factor(TCGA_BEAT_FLT3_ITD_labels[train_index])
y_test <- as.factor(TCGA_BEAT_FLT3_ITD_labels[-train_index])
...
# ggsave("plots/FLT3_ITD/train_test_split_proportions.png", device = "png", width = 15, height = 8,
...
# saveRDS(all_models, file = "OutputTables/FLT3_ITD_basic_all_models_CV_SMOTE.rds")
all_models <- readRDS("OutputTables/FLT3_ITD_basic_all_models_CV_SMOTE.rds")
...
# saveRDS(roc_df_list, "OutputTables/FLT3_ITD_basic_models_ROC_data.rds")
# write.table(performance_df, "OutputTables/FLT3_ITD_basic_all_models_CV_SMOTE_performance.txt",
#             col.names=TRUE, sep="\t")

roc_df_list <- readRDS("OutputTables/FLT3_ITD_basic_models_ROC_data.rds")
performance_df <- read.table("OutputTables/FLT3_ITD_basic_all_models_CV_SMOTE_performance.txt", header=T)
